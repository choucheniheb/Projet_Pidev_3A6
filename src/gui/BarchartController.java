...
public class BarchartController implements Initializable {
     ...
     */
    public void sendData(ObservableList<nourritures> no){
        this.lc=no;
        int[]  tableNote =new int[3];
        for (int i=0;i<lc.size();i++){
            if (lc.get(i).getPrix_nourriture()>15)
                tableNote[0]++;
            else if(lc.get(i).getPrix_nourriture()>10)
                tableNote[1]++;
            else
                tableNote[2]++;
